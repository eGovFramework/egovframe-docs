...
현재(2009년 5월) 기준으로, 본 온라인 매뉴얼의 설명이 되는 대상과 전자정부 표준프레임워크의 개발환경에 포함되어 제공되는 Subversion의 설치 버전은 다음과 같다.

* 서버 환경: Subversion 1.4.6

명시된 Subversion의 버전은 지속적인 전자정부 표준프레임워크 개발환경의 개선작업으로 인하여 향후 변경되어 배포될 수 있다

> Git 기반 소스관리도구를 사용하는 경우에는 [Git 기반 소스관리도구 운영](./git-based-scm.md)을 참고한다. 이 문서는 SVN 서버 운영 절차를 설명하며, Git 기반 서버 운영을 대체하지 않는다.
...
3. Atomic Commit
   * 변경점의 모임은, 그것 전체가 저장소에 완전하게 반영되는지, 전혀 반영되지 않는가 중 하나이다.
   * 개발자는 논리적으로 한 묶음의 변경을 만들어 Commit 할 수 있어 일부만이 저장소에 반영되어 버리는 문제가 발생하지 않는다.
4. 메타데이터 버전관리
   * 파일과 디렉터리는 각각 관련한 속성 키와 값의 조합이며 임의의 키/값의 조합을 생성해 보존할 수 있다.
