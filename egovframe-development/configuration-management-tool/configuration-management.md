...
| 이름       | 설명                                                                                                                                                                                                                                                                                | 라이선스 정책                                     |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| CVS        | 리비전 관리 시스템(Revision Control System)이라 불린 초기 버전 관리 시스템이 그 기원이며, 1989년 현재의 명칭인 CVS로 변경되었다. <br> 버전관리의 대상으로서 각각의 파일을 관리하지만, 프로젝트 전체는 관리할 수 없는 등 초창기 버전관리 도구로서의 여러 제약 사항을 지니고 있다. | GNU Public License(GPL) → Non-GNU License로 변경 |
...
* 분산형 워크플로우 : Git은 개발자 PC에 전체 이력을 가진 로컬 저장소를 두고 브랜치 생성, 커밋, 병합을 수행한 뒤 원격 저장소와 동기화할 수 있다.
* 브랜치 기반 협업 : Git은 기능 브랜치, 릴리스 브랜치, 핫픽스 브랜치 등 목적별 브랜치를 가볍게 만들 수 있어 병렬 개발과 코드리뷰 절차를 구성하기 쉽다.
* 리뷰 및 자동화 연계 : Git 기반 서버는 merge request/pull request, 보호 브랜치, 승인 규칙, CI/CD 파이프라인과 함께 사용되는 경우가 많다.

Subversion은 파일·디렉터리 단위 접근 제어와 중앙 집중형 운영이 필요한 환경에서 여전히 유효하다. 반면 Git은 브랜치 기반 협업, 코드리뷰, 자동화된 빌드·배포와 결합하기 쉬워 신규 개발과 민관 협업형 오픈소스 개발에 널리 사용된다. 따라서 표준프레임워크 기반 사업에서는 기관의 보안 정책, 협업 방식, 배포 자동화 수준에 따라 SVN 기반 운영과 Git 기반 소스관리도구 운영을 선택하거나 병행할 수 있다.

다음은 SVN 기반 운영과 Git 기반 서버 운영을 선택할 때 고려할 수 있는 기준이다.

| 구분 | SVN 기반 운영이 적합한 경우 | Git 기반 서버 운영이 적합한 경우 |
| --- | --- | --- |
| 저장소 구조 | 중앙 저장소 중심으로 단순한 개발 흐름을 유지해야 하는 경우 | 개발자별 로컬 저장소와 원격 저장소를 함께 활용해야 하는 경우 |
| 권한 정책 | 파일 또는 디렉터리 단위 접근 제어가 중요한 경우 | 저장소, 그룹, 브랜치, merge request/pull request 단위 권한과 승인 절차가 중요한 경우 |
| 협업 방식 | 커밋 중심의 중앙 집중형 형상관리가 충분한 경우 | 기능 브랜치, 코드리뷰, 병합 승인, 이슈 연계를 통한 협업이 필요한 경우 |
| 자동화 연계 | 별도 빌드 서버에서 주기적으로 소스를 가져와 빌드하는 경우 | push, tag, merge request/pull request 이벤트와 CI/CD 파이프라인을 연계하는 경우 |
| 전환 부담 | 기존 SVN 이력과 운영 절차를 그대로 유지해야 하는 경우 | 신규 사업이거나 SVN 이력·권한 정책을 재설계할 수 있는 경우 |

Git 기반 소스관리도구를 사용하는 경우 GitLab, Gitea, GitHub Enterprise 등 Git 서버 구현체를 기관 환경에 맞게 선택할 수 있다. 저장소 생성, 브랜치 전략, 코드리뷰, 보호 브랜치, 감사 로그, Jenkins 연계, SVN에서 Git으로의 전환 절차는 [Git 기반 소스관리도구 운영](./git-based-scm.md)을 참고한다.
