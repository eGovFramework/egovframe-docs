...
| 항목     | 설명                    | JDK                | 비고                           |
| -------- | ----------------------- | ------------------ | ------------------------------ |
| 개발환경 | Windows/Linux: Eclipse 4.35.0(2025-03)<br>macOS: Eclipse 4.38.0(2025-12)  | JDK 21 필요        |                                |
| 실행환경 | Spring 6.2.11         | JDK 17 이상의 환경 | JakartaEE, Servlet 5.0 이상<br>(단, Spring Boot는 Servlet 6.0 이상) |

### 버전별 환경 요구사항

- (2.7) JavaEE(J2EE), JDK 1.5 ~ 1.8의 환경 (단, 개발환경 2.7 이상에서는 JDK 1.6 이상 필요)
- (3.0) JavaEE(J2EE), JDK 1.6 ~ 1.8의 환경
- (3.5) JavaEE(J2EE), JDK 1.7 ~ 1.8의 환경 (단, 개발환경 3.5.1 부터 JDK 1.8 적용 가능)
- (3.6) JavaEE(J2EE), JDK 1.7 ~ 1.8의 환경
- (3.7) JavaEE(J2EE), JDK 1.7 ~ 1.8의 환경 (단, 개발환경 3.7 에는 JDK 1.8 필요)
- (3.8) JavaEE(J2EE), JDK 1.7 ~ 1.8의 환경 (단, 개발환경 3.8 에는 JDK 1.8 필요)
- (3.9) JavaEE(J2EE), JDK 1.7 ~ 1.8의 환경 (단, 개발환경 3.9 에는 JDK 1.8 필요)
- (3.10) JavaEE(J2EE), JDK 1.7 ~ 1.8의 환경 (단, 개발환경 3.10 에는 JDK 1.8 필요)
- (4.0) JavaEE(J2EE), Servlet 3.1 이상, JDK 1.8 이상의 환경 (단, 개발환경 4.0 부터 JDK 11 필요)
- (4.3) JavaEE(J2EE), Servlet 3.1 이상, JDK 1.8 이상의 환경 (단, 개발환경 4.3 부터 JDK 17 필요)
- (5.0) JakartaEE, Servlet 5.0 이상 (단, Spring Boot는 Servlet 6.0 이상), JDK 17 이상의 환경 (단, 개발환경 5.0 부터 JDK 21 필요)
...
설치한 구현도구의 플러그인이 최신 모듈을 사용할 수 있도록 [구현도구(implementation tool) 플러그인 업데이트](./egovframe-development/install-guide/implementation-tool-manual-install.md)를 참조하여 업데이트를 수행한다.
...
시작하기 개발환경은 실제 프로젝트 수행환경과 달리 Nexus를 이용하지 않고, 종속 라이브러리를 Maven 로컬 파일저장소에 수동으로 복사하여 개발환경을 구축한다. Maven의 로컬 파일 저장소를 설정하기 위하여 제공한 maven repository 파일을 임의의 디렉토리에 압축 해제하여 설치한다.

#### 종속라이브러리 설치순서

1. Maven 설정파일 및 종속라이브러리를 포함한 [maven repository](https://maven.egovframe.go.kr/publist/HDD1/public/egovframework_v5.0/2_DevelopmentEnvironment/gettingstarted/gettingstarted-maven-repository.zip)를 다운로드 한다.
...
> Setting.xml 참조: [Setting.xml](https://maven.egovframe.go.kr/publist/HDD1/public/egovframework_v5.0/2_DevelopmentEnvironment/gettingstarted/gettingstarted-settings-xml.zip)
...
프로젝트 생성 및 실행을 위하여 본 가이드는 [HelloWorld 프로젝트 파일](https://maven.egovframe.go.kr/publist/HDD1/public/egovframework_v5.0/2_DevelopmentEnvironment/gettingstarted/gettingstarted_helloworld.zip)을 제공한다. 아래의 순서에 따라 프로젝트를 생성한다.
...
1. 개발환경에서 egovframework.guide 프로젝트를 마우스 오른쪽 버튼으로 클릭하고 Run As > Maven install 를 클릭한다.

   ![Maven 빌드1](./images/maven-build-1.png)

2. 콘솔창에서 Maven 빌드의 결과를 확인한다. 메이븐 빌드는 프로젝트의 컴파일, 테스트, 패키징을 모두 수행하고 그 결과를 제공한다.
...
public class HelloWorldServiceImpl implements HelloWorldService {
	...
	public HelloWorldServiceImpl(String name) {
...
다음은 HelloWorld 서비스의 속성정의 파일이다. helloworld의 name 속성을 생성자 주입 방식으로 주입하여 서비스 구현클래스를 정의하고 있으며 해당 서비스의 name 속성을 "egov framework"로 선언하였다.
...
<beans xmlns="http://www.springframework.org/schema/beans"
	...
	<bean name="helloworld" class="egovframework.guide.helloworld.HelloWorldServiceImpl">
		<constructor-arg value="egov framework"/>
	</bean>
...
다음은 HelloWorld 서비스를 실행하기 위한 클라이언트 클래스이다.
...
다음은 HelloWorld 서비스 테스트 클래스의 속성정의 파일이다. helloworld의 name 속성을 생성자 주입 방식으로 주입하여 서비스 구현클래스를 정의하고 있으며 해당 서비스의 name 속성을 "eGovFrame"으로 선언하였다.
...
<beans xmlns="http://www.springframework.org/schema/beans"
	...
	<bean name="helloworld" class="egovframework.guide.helloworld.HelloWorldServiceImpl">
		<constructor-arg value="eGovFrame"/>
	</bean>
