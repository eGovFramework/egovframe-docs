...
사용자의 /세미나/강의/교육/회의/기타 일정에 대한 계획을 쉽고 편하게 관리할 수 있는 서비스로 월별일정보기, 주간별일정보기, 일별일정보기 기능을 제공한다.
...
관리자가 기존에 등록된 일정관리 정보를 리스트 형태로 조회하고, 등록 버튼을 클릭하여 등록 화면으로 이동할 수 있다.
...
수정 : 수정버튼 클릭 시 일정관리 수정 화면으로 이동한다.
...
일정관리에 관한 기본정보를 입력하여 저장한다. 입력명 우측의 빨간* 표시는 반드시 입력해야 할 항목을 표시한다.
...
입력한 일정관리 정보를 저장 처리한다. 입력명 우측의 빨간* 표시는 수정 시 반드시 입력해야 할 항목을 표시한다.
