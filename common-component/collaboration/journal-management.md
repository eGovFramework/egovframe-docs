...
사용자가 일정관리와 부서일정관리를 이용할 때 일정에 대한 기록을 남길 때 본 서비스를 이용한다.

```mermaid
flowchart LR
    L[일지관리 목록조회] -->|등록| R[일지관리 등록]
    L -->|목록 클릭| D[일지관리 상세조회]
    D -->|수정| U[일지관리 수정]
    D -->|삭제| L
    R --> L
...
| 테이블명 | 테이블명(영문) | 비고 |
| --- | --- | --- |
| 일지관리 | COMTNDIARYINFO | 일지를 관리한다. |

## 관련기능

일지관리 기능은 일지관리 목록조회, 일지관리 상세조회, 일지관리 등록, 일지관리 수정 기능으로 구성되어 있다.

### 일지관리 목록조회

#### 비즈니스 규칙

관리자가 기존에 등록된 일지관리 정보를 리스트 형태로 조회하고, 등록 버튼을 클릭하여 등록 화면으로 이동할 수 있다.
...
일지관리에 관한 기본정보를 입력하여 저장한다. 입력명 우측의 빨간* 표시는 반드시 입력해야 할 항목을 표시한다.
...
입력한 일지관리 정보를 저장 처리한다. 입력명 우측의 빨간* 표시는 수정 시 반드시 입력해야 할 항목을 표시한다.
