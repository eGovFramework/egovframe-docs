...
```mermaid
flowchart LR
    L[문자메시지 목록조회] -->|전송| S[문자메시지 전송]
    L -->|목록클릭| D[문자메시지 상세조회]
    S -->|M-Gov 연동 API| G([M-Gov 발송])
    G -->|스케줄러 결과수신| L
```
...
전송에 대한 결과는 별도의 스케줄러를 통해 반영된다. 해당 스케줄러를 등록하기 위해서는 …/spring/com/context-scheduling-cop-sms.xml(예시)에 다음과 같은 스케줄러를 등록한다.
