...
- ID Generation Service를 활용하기 위해서 Sequence 저장 테이블인 COMTECOPSEQ에 **FAQ_ID** 항목을 추가해야 한다. 
...
| 테이블명 | 테이블명(영문) | 비고 |
| --- | --- | --- |
| FAQ정보 | COMTNFAQINFO | 서비스 이용 시 빈번히 하는 질문 및 이에 대한 답변을 관리한다. |
...
#### 비즈니스 규칙
FAQ 목록조회에서 목록 클릭 시 이동되는 화면으로, FAQ에 대한 상세 정보를 보여준다.
...
#### 비즈니스 규칙
FAQ에 관한 기본 정보를 입력하여 저장 처리한다. 입력명 우측의 빨간 * 표시는 반드시 입력해야 할 항목을 의미한다. 
...
파일 첨부 시 '찾아보기'를 클릭하여 파일을 첨부할 수 있다. (최대 3개 가능 - 설정 가능)

![파일첨부](./images/faq-management-attach.gif)

목록: **FAQ 목록조회** 화면으로 이동한다. 
저장: 입력한 FAQ 정보들이 저장 처리된다. 

### FAQ 내용수정

#### 비즈니스 규칙
입력한 FAQ 정보들을 저장 처리한다. 입력명 우측의 빨간 * 표시는 수정 시 반드시 입력해야 할 항목을 의미한다. 
