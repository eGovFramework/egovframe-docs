...
설문항목관리 시스템 구축 시 사용되는 설문항목관리 기능을 제공하며, 설문조사 참여자가 설문에 참여할 때 설문항목관리 정보를 제공한다.

## 설명

```mermaid
flowchart LR
    L[설문항목관리 목록] -->|등록| R[설문항목관리 등록]
    L -->|목록 클릭| D[설문항목관리 상세조회]
    D -->|수정| U[설문항목관리 수정]
    D -->|삭제| L
    R --> L
    U --> L
...
| 설문항목관리 | COMTNQUSTNRIEM | 설문지 항목을 관리한다. |
...
관리자가 기존에 등록된 설문항목 정보를 리스트 형태로 조회하고, 등록 버튼을 클릭하여 등록 화면으로 이동할 수 있다.
...
목록 클릭: 설문항목 상세조회 화면으로 이동한다.

### 설문항목관리 등록

#### 비즈니스 규칙

설문항목에 관한 기본정보를 입력하여 저장한다. 입력명 우측의 빨간* 표시는 반드시 입력해야 할 항목을 표시한다.
...
입력한 설문항목 정보를 저장 처리한다. 입력명 우측의 빨간* 표시는 수정 시 반드시 입력해야 할 항목을 표시한다.
