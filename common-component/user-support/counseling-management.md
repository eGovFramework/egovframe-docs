...
## 개요
 ...
 상담관리는 일반 사용자가 상담 내역을 등록하고, 관리자가 등록된 상담에 답변을 작성·관리할 수 있도록 지원하는 기능이다.
...
상담관리 기능은 사용자 역할에 따라 구분된다. 일반 사용자는 상담 목록 조회, 상담 상세 조회, 상담 내역 등록·수정 기능을 사용할 수 있으며, 관리자는 상담 답변 목록 조회, 상담 답변 상세 조회, 상담 내역 답변 수정 기능을 사용할 수 있다.
...
#### 비즈니스 규칙
 ...
 조회 조건을 설정하여 상담 목록을 조회할 수 있으며, `등록` 버튼을 클릭하면 상담 등록 화면으로 이동한다.
...
|  |  |  | "CnsltManageDAO" | "selectCnsltListTotCnt" |
 ...
 상담 목록은 페이지당 10건씩 조회되며 페이징은 10페이지씩 이루어진다.
 ...
 - 조회: 상단에서 검색 조건을 선택하고 검색어를 입력한 후 `조회` 버튼을 클릭한다.
 - 등록: 상단의 `등록` 버튼을 클릭하면 상담 등록 화면으로 이동한다.
 - 상세조회: 목록에서 상담 제목을 클릭하면 상담 상세 조회 화면으로 이동한다.
...
| 삭제 | /uss/olp/cns/CnsltDtlsDelete.do | deleteCnsltDtls | "CnsltManageDAO" | "deleteCnsltDtls" |
 ...
 `수정` 또는 `삭제` 버튼을 클릭하면 작성 비밀번호를 확인한 후 상담 내용을 수정하거나 삭제할 수 있다.

 ![image](./images/uss-cnslt-password_confirm.png)

 - 수정: `수정` 버튼을 클릭하면 상담 내용을 수정할 수 있는 화면으로 이동한다.
 - 삭제: `삭제` 버튼을 클릭하면 삭제 여부를 확인하는 메시지가 표시되며, 확인 후 상담 내용을 삭제할 수 있다.
 - 목록: `목록` 버튼을 클릭하면 상담 목록 조회 화면으로 이동한다.

### 상담내역등록

#### 비즈니스 규칙

 상담에 필요한 기본 정보를 입력한 후 저장한다. 입력 항목 오른쪽의 빨간색 `*` 표시는 필수 입력 항목을 의미한다.
...
| 상세조회 | /uss/olp/cnm/CnsltAnswerDetailInqire.do | selectCnsltAnswerListDetail | "CnsltManageDAO" | "selectCnsltAnswerDetail" |
 ...
 - 수정: `수정`버튼 클릭 시 상담을 수정할 수 있는 화면으로 이동한다.
