...
```mermaid
flowchart LR
    L[저작권보호정책 목록조회] -->|등록| R[저작권보호정책내용등록]
    L -->|목록 클릭| D[저작권보호정책 상세조회]
    D -->|수정| U[저작권보호정책내용수정]
    D -->|삭제| L
    R -->|저장| L
    U -->|저장| L
...
#### 비즈니스 규칙
 ...
 조회조건으로 목록조회를 할 수 있고, 등록버튼을 클릭하여 저작권보호정책등록 화면으로 이동하여 저작권보호정책을 등록 처리할 수 있다.
...
|  |  | selectCpyrhtPrtcPolicyListTotCnt | "CpyrhtPrtcPolicyDAO" | "selectCpyrhtPrtcPolicyListTotCnt" |
 ...
 조회: 저작권보호정책을 조회하기 위해서는 상단의 검색조건을 선택 후 해당하는 검색문자를 입력 후 조회 버튼을 클릭한다.
 등록: 저작권보호정책을 등록하기 위해서는 상단의 등록 버튼을 통해서 저작권보호정책등록 화면으로 이동한다.
...
| 상세조회 | /uss/sam/cpy/CpyrhtPrtcPolicyDetailInqire.do | selectCpyrhtPrtcPolicyDetail | "CpyrhtPrtcPolicyDAO" | "selectCpyrhtPrtcPolicyDetail" |
 ...
 수정: 수정버튼 클릭 시 저작권보호정책을 수정할 수 있는 화면으로 이동한다.
