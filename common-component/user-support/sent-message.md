...
## 설명
 ...
 보낸쪽지함관리는 목록조회, 상세조회, 수신상태조회, 삭제의 기능을 제공한다.
...
```mermaid
flowchart LR
    L[보낸쪽지함관리 목록] -->|제목 클릭| D[보낸쪽지함관리 상세조회]
    L -->|받는사람/개봉여부 클릭| C[수신상태 조회]
    D -->|삭제| L
    C -->|삭제| L
```
...
| 보낸쪽지함관리 | COMTNNOTETRNSMIT | 보낸쪽지함을 관리하기 위한 속성정보를 정의하고, 관리한다. |
