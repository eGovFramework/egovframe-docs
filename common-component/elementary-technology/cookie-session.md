...
쿠키 및 세션 기능은 다음과 같은 상황에서 활용할 수 있다.

- 로그인 상태 유지
- 사용자 설정 정보 저장
- 장바구니와 같은 사용자별 데이터 관리
- 사용자 인증 및 권한 관리
- 웹 애플리케이션의 상태 정보 유지
...
쿠키에 정보를 저장, 조회 및 삭제하는 기능을 제공한다.
브라우저에 저장된 쿠키를 활용하여 사용자 정보를 유지하거나 필요한 데이터를 관리할 수 있다.
...
| --- | --- | --- |
| 저장 위치 | 클라이언트(브라우저) | 서버 |
| 주요 용도 | 사용자 설정 및 간단한 정보 저장 | 로그인 정보 및 사용자 상태 관리 |
| 유지 기간 | 설정된 만료 시간까지 유지 | 브라우저 종료 또는 세션 만료 시 종료 |

### 세션 처리

세션에 객체 정보를 저장, 조회 및 삭제하는 기능을 제공하며,
`HttpServletRequest` 객체의 `HttpSession`을 이용하여 세션 정보를 관리한다.
