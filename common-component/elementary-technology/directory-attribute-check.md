...
디렉토리 속성정보 체크 기능은 다음과 같은 상황에서 활용할 수 있다.

- 애플리케이션이 사용하는 디렉토리의 존재 여부와 상태를 확인하는 경우
- 파일 업로드 또는 로그 저장 경로의 읽기·쓰기 권한을 점검하는 경우
- 디렉토리 용량을 확인하여 저장 공간을 관리하는 경우
- 운영 서버의 디렉토리 소유자와 접근 권한을 확인하는 경우

## 설명

지정한 디렉토리 경로를 기준으로 이름, 최종 수정일자, 소유자, 접근 권한 및 용량 등의 속성정보를 조회한다.
이를 통해 애플리케이션에서 사용하는 디렉토리의 상태와 사용 가능 여부를 확인할 수 있다.
...
- 조회 대상 경로는 애플리케이션이 접근할 수 있는 유효한 디렉토리 경로여야 한다.
- 운영체제와 실행 계정의 권한에 따라 조회 결과가 달라질 수 있다.
- `getDirectorySize`는 하위 디렉토리와 파일을 함께 조회하므로, 대상 디렉토리의 크기가 큰 경우 처리 시간이 증가할 수 있다.
