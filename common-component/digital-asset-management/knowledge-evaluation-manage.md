...
## 설명
 ...
 지식평가관리는 필요지식을 생성하여 시스템에 등록할 때, 전문가의 평가를 받기 위한 목적으로 지식평가의 수정, 삭제, 상세조회, 목록조회의 기능을 수반한다.
...
| JSP | /WEB-INF/jsp/egovframework/com/dam/app/EgovComDamAppraisalModify.jsp | 지식평가 수정을 위한 jsp페이지 |
| JSP | /WEB-INF/jsp/egovframework/com/dam/app/EgovComDamAppraisalDetail.jsp | 등록된 지식평가를 조회하기 위한 jsp페이지 |
...
```mermaid
flowchart LR
    L[지식평가 목록조회] -->|지식명 클릭| D[지식평가 상세조회]
    D -->|평가| U[지식평가 수정]
    U -->|저장| L
    D -->|삭제| L
```
