...
## 개요
 ...
 배치결과관리는 시스템에서 주기적으로 실행하는 배치결과를 등록하는 기능을 제공한다.

## 설명

 배치결과관리는 배치결과를 등록하기 위한 목적으로 배치결과의 조회, 삭제, 목록조회의 기능을 수반한다.

 ① 배치결과목록조회 : 배치결과로 정의된 정보를 최근 등록 순서대로 조회하고, 그 결과 목록을 화면에 반영한다.
 ② 배치결과삭제 : 기 등록된 배치결과정보를 삭제한다.
 ③ 배치결과조회 : 등록된 배치결과정보를 조회한다.

```mermaid
flowchart LR
    L[배치결과 목록조회] -->|상세조회| D[배치결과 상세조회]
    D -->|삭제| X[배치결과 삭제]
    X --> L
```
