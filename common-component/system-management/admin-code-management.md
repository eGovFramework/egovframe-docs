...
### 패키지 참조 관계
 ...
 행정코드관리 패키지(adc)는 요소기술의 공통 패키지(cmm)에 대해서만 직접적인 함수적 참조 관계를 가진다. 하지만, 컴포넌트 배포 시 오류 없이 실행되기 위하여 패키지 간의 참조관계에 따라 법정동코드수신 패키지(acr), 달력 패키지와 함께 배포 파일을 구성한다.
