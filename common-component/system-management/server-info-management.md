...
```mermaid
flowchart LR
    HL[서버H/W 목록조회] -->|등록| HR[서버H/W 등록]
    HL -->|상세조회| HD[서버H/W 상세조회]
    HR --> HL
    HD -->|수정| HU[서버H/W 수정]
    HU --> HL
    HD -->|삭제| HL
    SL[서버S/W 목록조회] -->|등록| SR[서버S/W 등록]
    SL -->|상세조회| SD[서버S/W 상세조회]
    SR --> SL
    SD -->|수정| SU[서버S/W 수정]
    SU --> SL
    SD -->|삭제| SL
    SL -->|서버H/W 등록/미등록| HE[서버H/W 등록·삭제]
    HE --> SL
```
...
| Action | URL | Controller method | QueryID |
| --- | --- | --- | --- |
| 조회 | /sym/sym/srv/selectServerList.do | selectServerList | "serverDAO.selectServerList" |
