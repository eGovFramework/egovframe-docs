...
| indicator         | Ajax 요청중일 때 보여줄 표시.                                                           | no   |
...
결과 데이터를 AjaxXmlBuilder를 이용해서 변환하는 작업을 View에서 할 수도 있지만, View의 개수가 기능 단위로 추가될 수도 있으므로,
...
부서정보 페이지에서 각 상위부서에 속한 하위부서리스트를 보여줄 때, tab으로 처리해서 보여준다.
