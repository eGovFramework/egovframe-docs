...
Integration 서비스 Metadata의 물리모델은 논리모델을 실제 물리적인 DB로 구현하기 위한 모델로서, 아래 물리ERD는 Oracle DB를 가정하여 작성된 것이다. 물리모델은 Hibernate 등과 같은 Object Relational Mapping(ORM)을 사용하여 접근하는 것을 고려하여 ([ORM with Spring - Spring Framework 6.2](https://docs.spring.io/spring-framework/reference/6.2/data-access/orm.html) 참조), 복수의 Attribute를 Identifier로 갖는 Entity를 Table로 변환할 때 Surrogate Key를 도입하고, 기존 Identifier는 Unique Constraints로 적용하여 정의되었다.
...
| 모듈ID | BeanId | VARCHAR2(40) | 알파벳 대소문자, 숫자, ‘_’ 로 구성된 길이 1 ~ 40 사이의 문자열<br/>Ex) serviceProviderA, bean_123	 | • 모듈ID는 전자정부 개발프레임워크의 기반이 되는 Spring Framework에 등록되는 Bean Id이다. ([The IoC Container - Spring Framework 6.2](https://docs.spring.io/spring-framework/reference/6.2/core/beans.html), [Bean Overview](https://docs.spring.io/spring-framework/reference/6.2/core/beans/definition.html) 참조) |
