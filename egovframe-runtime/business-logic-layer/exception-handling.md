...
description: 전자정부 표준프레임워크 기반 시스템에서 Exception 처리는 AOP를 이용해 비즈니스 로직과 분리된 After throwing advice로 정의되며, Exception에 따른 적절한 대응을 목표로 한다. Spring Framework 6.2 AOP 및 Controller Advice와 정합성을 유지한다.
...
본 문서는 [Spring Framework 6.2 - Aspect-Oriented Programming (AOP)](https://docs.spring.io/spring-framework/reference/6.2/core/aop.html) 및 [Controller Advice](https://docs.spring.io/spring-framework/reference/6.2/web/webmvc/mvc-controller/ann-advice.html)와의 관계를 고려하여 작성하였다. Spring MVC 기반 애플리케이션에서는 **@ControllerAdvice**와 **@ExceptionHandler**를 사용해 컨트롤러 전역 예외 처리를 할 수 있으며, 표준프레임워크의 Exception Handling은 **서비스 계층**에서 AOP After-throwing 방식으로 후처리 로직을 적용하는 보완적 접근이다.
...
AOP의 도움을 받아 비즈니스 POJO와 분리되어 **After throwing advice**로 정의하였다. Spring Framework 6.2에서 AOP는 **schema 기반** 또는 **@AspectJ** 스타일로 작성할 수 있으며, 횡단 관심사(예: 예외 후처리)의 모듈화에 사용된다.<br/>
...
Bean 정의 방식은 [The IoC Container - Spring Framework 6.2](https://docs.spring.io/spring-framework/reference/6.2/core/beans.html) 및 [Dependencies](https://docs.spring.io/spring-framework/reference/6.2/core/beans/dependencies.html)를 참조한다.
...
leaveaTrace·TraceHandler 관련 Bean 정의는 [The IoC Container](https://docs.spring.io/spring-framework/reference/6.2/core/beans.html)를 참조한다.
...
- [The IoC Container - Spring Framework 6.2](https://docs.spring.io/spring-framework/reference/6.2/core/beans.html): Bean 정의, XML/Java 설정
- [Spring Framework 6.2 - Aspect-Oriented Programming with Spring](https://docs.spring.io/spring-framework/reference/6.2/core/aop.html): AOP 개념, schema 기반·@AspectJ 지원, 선언적 서비스
- [Spring Framework 6.2 - Controller Advice](https://docs.spring.io/spring-framework/reference/6.2/web/webmvc/mvc-controller/ann-advice.html): @ControllerAdvice, @ExceptionHandler를 이용한 전역 예외 처리
- [Spring Framework 6.2 - Declarative Transaction Management](https://docs.spring.io/spring-framework/reference/6.2/data-access/transaction/declarative.html): AOP 기반 선언적 트랜잭션(예외 후처리와 별개)
- [exception-handling-framework-for-j2ee](https://www.oreilly.com/radar/?page=1)
- Effective Java (Joshua Bloch): Chapter 8 예외 처리