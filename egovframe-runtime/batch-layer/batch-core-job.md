...
title: Job
linkTitle: "Job"
description: Job은 배치 작업 전체를 캡슐화하는 개념으로, 여러 Step을 포함하며 배치 작업 실행을 의미한다. Job은 각 JobParameters에 따라 JobInstance를 생성하고, Job 시도마다 JobExecution을 생성하여 작업을 처리한다. Spring Batch 5.2 도메인 언어와 정합성을 유지한다.
...
본 문서는 [Spring Batch 5.2 - Configuring and Running a Job](https://docs.spring.io/spring-batch/reference/5.2/job.html) 및 [The Domain Language of Batch](https://docs.spring.io/spring-batch/reference/5.2/domain.html)와 정합성을 유지한다.

## 개요

Job은 배치작업 전체의 중심 개념으로 배치작업 자체를 의미한다. Job은 실제 프로세스가 진행되는 [Step](./batch-core-step.md) 들을 최상단에서 포함하고 있으며, Job의 실행은 배치작업 전체의 실행을 의미한다. Spring Batch 5.2에서 Job은 Step 인스턴스의 **컨테이너**이며, 논리적으로 하나의 흐름에 속하는 여러 Step을 조합하고, 재시작 가능 여부 등 Step에 공통으로 적용할 속성을 설정한다.
...
- ex) FlatFileItemWriter  
  resource에 JobParameter의 'outputFile' 키를 통해 출력 경로를 지정할 수 있다.
...
Job은 XML 또는 Java 설정으로 표현하며, 이 설정 파일은 “Job 설정”이라 한다.

#### Java 설정

Spring Batch는 `JobBuilder`를 사용한 Java 기반 Job 설정을 제공한다. `@EnableBatchProcessing` 또는 `DefaultBatchConfiguration` 상속 시 `JobRepository`를 주입받아 사용한다.
...
재시작 불가 Job은 `preventRestart()`로 설정한다.
...
- [Spring Batch 5.2 - The Domain Language of Batch (Job)](https://docs.spring.io/spring-batch/reference/5.2/domain.html#job)
- [Spring Batch 5.2 - Configuring and Running a Job](https://docs.spring.io/spring-batch/reference/5.2/job.html)