...
**전자정부 프레임워크**에서는 스프링에서 제공하는 Apache Commons FileUpload API를 이용하여 파일 업로드를 처리하는 [CommonsMultipartResolver](https://docs.spring.io/spring-framework/docs/6.2.x/javadoc-api/org/springframework/web/multipart/commons/CommonsMultipartResolver.html) 클래스를 제공하며, Servlet 3.0+ 환경에서는 [StandardServletMultipartResolver](https://docs.spring.io/spring-framework/reference/6.2/web/webmvc/mvc-controller/multipart.html)도 사용할 수 있다. 클래스를 제공하고 다음과 같이 설정파일에 CommonsMultipartResolver를 빈으로 등록하여 준다.
...
Spring에서 multipart를 사용한 파일 업로드에 대해서는 [Spring Framework 6.2 - Multipart Resolver](https://docs.spring.io/spring-framework/reference/6.2/web/webmvc/mvc-controller/multipart.html)에서 자세히 다루므로 본 매뉴얼에서는 실행환경 관점만 설명한다.
