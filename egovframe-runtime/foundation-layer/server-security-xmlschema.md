...
| jdbcUsersByUsernameQuery | 인증에 사용되는 query | 선택 | default : “select user\_id, password, enabled, users.\* from users where user\_id = ?” |
| jdbcAuthoritiesByUsernameQuery | 인증된 사용자의 권한(authority) 조회 query | 선택 | default : “select user\_id, authority from authorites where user\_id = ?” |
